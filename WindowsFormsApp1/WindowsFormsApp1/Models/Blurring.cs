...
{
    class Blurring : IProcessingImage
    {
        private readonly Bitmap _image;
        private readonly double[,] _kernel;

        public Blurring(Bitmap image)
        {
        ...
        {
            Bitmap blurringImage = AlgorithmWithKernel(_image);

            int counter = 0;
            while (counter++ < 5)
            {
                blurringImage = AlgorithmWithKernel(blurringImage);
            }

            return blurringImage;
        }

        public Bitmap AlgorithmWithKernel(Bitmap image)
        {
            Bitmap imageModified = new Bitmap(image.Width + 2, image.Height + 2);

            for (int i = 1; i < imageModified.Width - 1; i++)
                for (int j = 1; j < imageModified.Height - 1; j++)
                    imageModified.SetPixel(i, j, image.GetPixel(i - 1, j - 1));
